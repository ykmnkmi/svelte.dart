...
void main() {
  ...
  test('should retain manual &ngsp; inserts', () {
    expect(parseAndMinifiy(r'<div>&ngsp;</div>'), '<div> </div>');
  });

  test('should retain manual &#32; inserts', () {
    expect(parseAndMinifiy(r'<div>&#32;</div>'), '<div> </div>');
  }, skip: 'Not yet supported');

  test('should retain single whitespaces around tags', () {
    expect(parseAndMinifiy('Foo <strong>Bar</strong> Baz'), 'Foo <strong>Bar</strong> Baz');
  });
  ...
  group('whitespace', () {
    ...
    test('should leave a space between potentially inline elements', () {
      expect(parseAndMinifiy(r'<span>Hello</span> <span>World</span>!'), '<span>Hello</span> <span>World</span>!');
    });

    test('should not leave a space between potentially block elements', () {
      expect(parseAndMinifiy(r'<div>Hello</div> <div>World</div>!'), '<div>Hello</div><div>World</div>!');
    });
...
String parseAndMinifiy(String template) {
  var nodes = parse(template, sourceUrl: 'whitespace_test.dart') as List<Standalone>;
  var buffer = StringBuffer();

  for (var node in const MinimizeWhitespaceVisitor().visitAllRoot(nodes)) {
    buffer.write(humanize(node));
  }

  return buffer.toString();
}

String humanize(Node astNode) {
  return astNode.accept(const HumanizingTemplateAstVisitor())!;
}
