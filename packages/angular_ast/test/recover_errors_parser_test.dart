...
void main() {
  ...
  test('Should drop invalid decorators on ng-container', () {
    ...
    expect(attrException.errorCode, ParserErrorCode.INVALID_DECORATOR_IN_NGCONTAINER);
    expect(attrException.offset, 27);
    expect(attrException.length, 12);

    final propException = exceptions[1];
    expect(propException.errorCode, ParserErrorCode.INVALID_DECORATOR_IN_NGCONTAINER);
    expect(propException.offset, 40);
    expect(propException.length, 13);

    final eventException = exceptions[2];
    expect(eventException.errorCode, ParserErrorCode.INVALID_DECORATOR_IN_NGCONTAINER);
    expect(eventException.offset, 54);
    expect(eventException.length, 14);

    final letException = exceptions[3];
    expect(letException.errorCode, ParserErrorCode.INVALID_DECORATOR_IN_NGCONTAINER);
    expect(letException.offset, 69);
    expect(letException.length, 14);

    final refException = exceptions[4];
    expect(refException.errorCode, ParserErrorCode.INVALID_DECORATOR_IN_NGCONTAINER);
  ...
  test('Should resolve dangling close template', () {
    ...
    expect((template as EmbeddedTemplateAst).closeComplement!.isSynthetic, false);
  ...
  test('Should drop invalid attrs in ng-content', () {
    var html = '<ng-content bad = "badValue" select="*" [badProp] = "badPropValue" #validRef></ng-content>';
    var asts = parse(html);
    expect(asts.length, 1);
